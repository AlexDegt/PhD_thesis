...
def fir_filtering_matrix_conv(P, w, mode='same'):
    """
        Function filters columns of matrix P,
        filter with impulse response w
    ...
    P_filtered = []
    for i in range(col_num):
        P_filtered.append(signal.convolve(P[:, i], w, mode=mode))
    P_filtered = np.array(P_filtered).T
    return P_filtered
...
def lut_matrix_generate(x, nspl, pwr = 1):
    """

...
sys.path.insert(1, curr_dir+r'/CommPy-master/commpy')
sys.path.insert(1, curr_dir+r'/CommPy-master')
...
DATA_PR_122K_SAMP_NONAVER  = '122880_pr_navr'
DATA_NOPR_122K_SAMP_NONAVER  = '122880_npr_navr'
DATA_PR_122K_SAMP_AVER  = '122880_pr_avr'
DATA_NOPR_122K_SAMP_AVER  = '122880_npr_avr'
...
""" RB numbers for masters thesis 2022-05-31 """
RB_mthesis_22_05_31 = np.array([1, 25, 50, 75, 100])
...
START_mthesis_22_05_31 = np.array(['50', '37', '25', '12', '0'])
""" 
...
""" VGA power for masters thesis 2022-05-31 """
VGA_pwr_aver_pr_22_05_31 = np.array([-34, 0, 0, 0, 0, -29, 0, 0, 0, 0, -25, 0, 0, 0, 0, -21, 0, 0, 0, -16])
""" 
...
'''
    Consolse arguments keys
    How to apply to enum example:
        argv_keys.FIR_LAYER_STEP_LOW.name
        argv_keys.FIR_LAYER_STEP_LOW.value
'''
class alg_keys(Enum):
    DS_LEN = '-dslen'
    INITIAL_FIR_COEF = '-incfir'
    INITIAL_LUT_COEF = '-inclut'
    INITIAL_STATE_OBJ = '-inst'
    FIR_LAYER_STEP_LOW = '-musfir'
    FIR_LAYER_STEP_HIGH = '-muffir'
    FIR1_STEP_LOW = '-musfir1'
    FIR1_STEP_HIGH = '-muffir1'
    FIR2_STEP_LOW = '-musfir2'
    FIR2_STEP_HIGH = '-muffir2'
    FIR_LAYER_LEAK = '-lamfir'
    FIR_REGUL = '-reg'
    FIR_REGUL_RATIO = '-regrat'
    FIR_ACCUM_DELAY = '-delfir'
    FIR_LAYER_MOM_LOW = '-momsfir'
    FIR_LAYER_MOM_HIGH = '-momffir'
    LUT_LAYER_MOM_LOW = '-momslut'
    LUT_LAYER_MOM_HIGH = '-momflut'
    LUT_LAYER_STEP_LOW = '-muslut'
    LUT_LAYER_STEP_HIGH = '-muflut'
    LUT_LAYER_LEAK = '-lamlut'
    LUT_ACCUM_DELAY = '-dellut'
    FIR_DCD_NU = '-nudcdfir'
    FIR_DCD_MB = '-mbdcdfir'
class gen_ds_keys(Enum):
    POWER_RAMP = '-pwrp'
    DATA_AVERAGE = '-avrds'
    SIGNAL_LENGTH = '-siglen'
    PHASE_JUMP_DISTR = '-pj'
    PHASE_JUMP_DISTR_PARAM = '-pjdist'
    AWGN_EXPECT = '-awgnex'
    AWGN_DEVIAT = '-awgndev'
class model_keys(Enum):
    FIR_ORDER = '-ord'
    FIR1_ORDER = '-ord1'
    FIR2_ORDER = '-ord2'
    HAMMERST_POWER = '-pwr'
    ...
    SPLINE_NUM_1 = '-nspl1'
    SPLINE_NUM_2 = '-nspl2'
    LUT_RATIO_1AXIS = '-rspl'
    LUT_RATIO_2AXIS = '-rsplrb'
class save_keys(Enum):
    SAVE_RESULTS = '-svr'
class sym_keys(Enum):
    LEARN_SWITCH = '-lswitch'
    SWITCHED_LUT_LAYER_STEP_LOW = '-muswluts'
    SWITCHED_FIR_LAYER_STEP_LOW = '-muswfirs'
    SWITCHED_LUT_LAYER_STEP_HIGH = '-muswlutf'
    SWITCHED_FIR_LAYER_STEP_HIGH = '-muswfirf'
    STOP_SAMPLE = '-stpsmp'
    GSIM_USE = '-gsim'
    DESCRIPTION = '-dspt'
    

DS_PARAM_TXT_TITLE = 'RB_and_path'
...
def create_rx_signal(sig_size, rb_num=100, rb_subcar_num=12, qam_order=10, guard_len=288, nfft=8192, rx_bit=[]):
    """
        Function creates received downlink signal, that
        need to be filtered from IMD
    """
    subcar_num = rb_num*rb_subcar_num
    n_ofdm_sym = int(np.ceil(sig_size/(nfft+guard_len)))
    qam = QAMModem(2**qam_order)
    if rx_bit == []:
        rx_bit = np.random.randint(0, 2, size = n_ofdm_sym*subcar_num*qam_order)
    else:
        rx_bit = rx_bit[0:n_ofdm_sym*subcar_num*qam_order]
    qam_map = qam.modulate(rx_bit)
    # print(len(qam_map))
    qam_map_sc = qam_map.reshape(subcar_num, n_ofdm_sym)
    ofdm_sym = mod.ofdm_tx(qam_map_sc, int(nfft), subcar_num, guard_len)
    ofdm_sym /= np.max(np.abs(ofdm_sym))
    # ofdm_sym /= np.max(np.abs(get_psd(ofdm_sym)))
    ofdm_sym = ofdm_sym[:sig_size]
    return rx_bit, ofdm_sym
...
def  plot_stat(e_max, e_min, e_mean, e_accur, rb_num, block_len=960, block_num=128, alpha=0.3):
    """
    """
    rb_ind = RB2ind(rb_num)
    e_max_rt = []
    e_accur_high_rt = []
    e_mean_rt = []
    e_accur_low_rt = []
    ...
    for i in range(block_num):
        e_max_rt.append(sl.nmse_rt(d, e_max[rb_ind, i*block_len:(i+1)*block_len]))
        e_accur_high_rt.append(sl.nmse_rt(d, e_accur[rb_ind, 0, i*block_len:(i+1)*block_len]))
        e_mean_rt.append(sl.nmse_rt(d, e_mean[rb_ind, i*block_len:(i+1)*block_len]))
        e_accur_low_rt.append(sl.nmse_rt(d, e_accur[rb_ind, 1, i*block_len:(i+1)*block_len]))
        e_min_rt.append(sl.nmse_rt(d, e_min[rb_ind, i*block_len:(i+1)*block_len]))
        
    e_max_rt = np.array(e_max_rt)
    e_accur_high_rt = np.array(e_accur_high_rt)
    e_mean_rt = np.array(e_mean_rt)
    e_accur_low_rt = np.array(e_accur_low_rt)
    e_min_rt = np.array(e_min_rt)

    # plt.figure(1)
    # plt.plot(20*np.log10(max_arr[rb_ind, :]))
    # plt.plot(20*np.log10(e_accur[rb_ind, 0, :]))
    # plt.plot(20*np.log10(mean_arr_abs[rb_ind, :]))
    # plt.plot(20*np.log10(e_accur[rb_ind, 1, :]))
    # plt.plot(20*np.log10(min_arr[rb_ind, :]))

    plt.figure(3)
    plt.fill_between(np.arange(128), e_max_rt, e_min_rt, alpha=0.3, color='y', linestyle='dashed')
    plt.plot(e_mean_rt, color='black')
    plt.fill_between(np.arange(128), e_accur_high_rt, e_accur_low_rt, alpha=0.3, color='red', linestyle='dashed')
...
def nzero_2ndax_lut(dw_lut, nspl):
    non_zero_spl = np.arange(0, nspl*nspl)
    for i in range(nspl):
        if (dw_lut[i*nspl:(i+1)*nspl, 0]==np.zeros((nspl,), dtype='complex128')).all():
            non_zero_spl[i*nspl:(i+1)*nspl] = -1
    non_zero_spl=non_zero_spl[non_zero_spl!=-1]
    return non_zero_spl

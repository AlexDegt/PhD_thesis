...
folder = os.path.join(os.getcwd(), "signals", "bgd_12_pow_dim_inp_stand_m1_1_50_epochs_mu_1e_4_save_weights")

folder_ls = os.path.join(os.getcwd(), "signals", "ls_12_pow_dim_inp_stand_m1_1_reg_m13")

add_to_name = "_2871_iter", "_5060_iter", "_7636_iter", ""

for add in add_to_name:

    tx_train = np.load(os.path.join(folder, f'x{add}.npy'))
    pa_out_train = np.load(os.path.join(folder, f'd{add}.npy'))
    model_out_train = np.load(os.path.join(folder, f'y{add}.npy'))
    tx_test = np.load(os.path.join(folder, f'x_test{add}.npy'))
    pa_out_test = np.load(os.path.join(folder, f'd_test{add}.npy'))
    model_out_test = np.load(os.path.join(folder, f'y_test{add}.npy'))

...
Смешанный метод Ньютона~--- модификация метода Ньютона~\eqref{newton_method_full} в условиях голоморфности вектора ошибки $\bit{e}=\bit{e}(\bit{z}m \bit{x})$. Иначе говоря смешанный метод Ньютона

Для корректной работы алгоритма вектор ошибки $\bit{e}$~\eqref{mse} должен быть голоморфным относительно параметров модели \bit{z}, то есть должен не зависеть от сопряжённых параметров $\bit{z}^*$: $\bit{e}=\bit{e}(\bit{x}, \bit{z})$. В этом случае MNM обладает свойством отталкивания от седловых точек~\cite{mixed_newton}.

Поскольку алгоритм MNM использует только смешанный гессиан $H_{\bit{z}^*, \bit{z}}J$ для обучения параметров модели~\cite{mixed_newton}, то разностное уравнения смешанного метода Ньютона может быть представлено следующим выражением:
\begin{equation}
	\bit{z}_{k+1}=\bit{z}_k-\mu (H_{\bit{z}^*, \bit{z}} J)^{-1}(D_{\bit{z}^*}J)^T,
	\label{mixed_newton_eq}
\end{equation}
где $J$~--- MSE, определенная выражением \eqref{mse_block}.

В то же время, согласно обозначению \eqref{hessian_calc} и выражению для градиента \eqref{grad}, смешанный гессиан можно рассчитать следующим способом:
\begin{equation}
	H_{\bit{z}^*, \bit{z}}J=D_{\bit{z}}(D_{\bit{z}^*}J)^T=-D_{\bit{z}}((D_{\bit{z}}\bit{y})^H\bit{e}).
\end{equation}
Поскольку вектор выхода модели \bit{y} является голоморфным относительно параметров модели, то смешанный гессиан может быть представлен как:
\begin{equation}
	H_{\bit{z}^*, \bit{z}}J=(D_{\bit{z}}\bit{y})^HD_{\bit{z}}\bit{y}.
	\label{mixed_hessian}
\end{equation}
Таким образом, учитывая выражения \eqref{grad}, \eqref{mixed_hessian} и \eqref{mixed_newton_eq}, алгоритм смешанного метода Ньютона можно представить следующим разностным уравнением:
\begin{equation}
	\bit{z}_{k+1}=\bit{z}_k-\mu ((D_{\bit{z}}\bit{y})^HD_{\bit{z}}\bit{y})^{-1}(D_{\bit{z}}\bit{y})^H\bit{e}.
	\label{mixed_newton_eq_jac}
\end{equation}

Отдельно отметим, что сложность хранения гессиана оценивается как $o(M^2)$ \cite{matrix_comput}, если $M\times M$ -- размерность гессиана. Обращение гессиана может быть сведено к решению системы из $M$ линейных уравнений. Вычислительная сложность такой операции может быть оценена как $o(M^3)$ \cite{matrix_comput}.

Кроме того, отметим, что разностное уравнение \eqref{diff_equat_newton_hammerst} метода Ньютона применительно к модели Гаммерштейна также представляет собой итеративную LS-оценку.
